...
#define CacheSize 1024
	...
	int	 iPickSlipNumberBuf,iRecordsF4211Read,iLineaEscogida,iUPClenBuf,iSCClenBuf, 
		 iErrorCode,iErrorCode1,iDbgFlg,iItemFoundInCache,iRecordsAvailCache,iEditLineLines,iCantLotesProc,iNumLotes,i,
	...
	int	 iIdxCo = 0, iIdxLoc = 0, iIdxDT = 0, iIdxDoc = 0,
		 iIdxProd = 0, iIdxCant = 0, iIdxLot = 0, iIdxUM = 0, iIdxFVcto = 0;
...
//	iCantDecQty = atoi(dsGetDataDictionaryDefinition.mnDisplayDecimals.String);
	...
	do {  //Loop para obtener un almacén correcto y fechas...
	...
	if (iDbgFlg > 0) jdeFprintf(dlg,_J("***LszDocBranchPlantBuf (%ls), (szCompany:%ls)...\n"), 
										LszDocBranchPlantBuf,dsRetrieveCompanyFromBusUnit.szCompany);
	...
	do{ //while (TRUE);
		...
		do {  //while (iRecordsF4211Read == 0); Loop para obtener un pick slip correcto...
			...
			idResult = JDB_SetSelection(hRequest,lpSelectF4211,(short)(8),JDEDB_SET_REPLACE);
			if (idResult == JDEDB_FAILED){
				iErrorCode = 305;
				...
				if (idResult == JDEDB_FAILED)jdeFprintf(dlg,_J("***JDB_Fetch (EOF F4211) (%d) ...\n"),iRecordsF4211Read);
					...
					if (iDbgFlg > 0){
						...
						jdeFprintf(dlg,_J("***Cache[%d] Order Company(%ls)...\n"),iRecordsF4211Read,
								dsF4211Buf[iRecordsF4211Read].sdkcoo);
		...
		do { //while (iRecordsAvailCache > 0); Procesar mientras haya registros en el Cache... Cambiado a TRUE...
			...
			do {//while (iItemFoundInCache == 0); Solicita producto a procesar...
				...
				do { //Loop para validar codigo barra...
					...
					if (strlen(szCamposContenido[0]) == 0) {
							iSalir++;
							if (iDbgFlg > 0){
								jdeFprintf(dlg,_J("***Break de Etiqueta...\n"));
								jdeFflush(dlg);
							}
							break;
					} 
				...
				iItemFoundInCache = 0;
				for (i = 0;(i < iRecordsF4211Read) && (iItemFoundInCache == 0); i++){//Ahora queremos encontrar el producto en el pedido...					
					if ((jdeStrcmp(LszLineItemBuf,dsF4211Buf[i].sdaitm) == 0) && 
						(dsF4211Buf[i].iStatusRegistro != 2)) { //Mostrar registros no procesados
						if (iDbgFlg > 0) {
							jdeFprintf(dlg,_J("***dsF4211Buf[%d].sdaitm (%ls)...\n"),i,dsF4211Buf[i].sdaitm);
							jdeFflush(dlg);
						}
						jdeFprintf(dlg,_J("%d)Producto(%ls-%ls)...\n"),i,dsF4211Buf[i].sdaitm,dsF4211Buf[i].sddsc1);
						jdeToUnicode(LszString01,dsF4211Buf[i].sdsoqs.String,DIM(LszString01),UTF8);
						jdeFprintf(dlg,_J("%d)Cantidad(%ls/%ls) Ubicacion(%ls)...\n"),i,LszString01,dsF4211Buf[i].sduom,dsF4211Buf[i].sdlocn);
						iLineaEscogida = i;//Si es la única linea del Cache, esta es...Si no, agarramos el primero...
						dsF4211Buf[i].iStatusRegistro = 1;
						iItemFoundInCache++;
					}
				}
			} while (iItemFoundInCache == 0);//No se encontró producto en Cache, solicitar otro...
			...
			//OWDCmp90 (iDbgFlg, dlg);//Limpiamos la 3da pantalla para mostrar la 4ta y 5ta...

			if (iDbgFlg > 0){
				jdeFprintf(dlg,_J("***Inicializando 4ta y 5ta pantallas datos entrada Línea...\n"));
				jdeFflush(dlg);
			}
			...
			//***Lote 
			jdeToUnicode(LszLineLotBuf,szCamposContenido[iIdxLot],30,UTF8);
...
//INICIO Modificación - 08/02/2010
//Modificado:	Luis Capriles,		Fecha:	08/02/2010 - Cambio orden datos detalle.

			int iIdxCant = 0, iIdxUbic = 0;
			...
			szCamposEtiquetas[iIdxUbic] = "Ubicacion....";
			szCamposEtiquetas[iIdxCant] = "Cantidad...";

			iCamposCantidad = 2;//2 campos a desplegar

			//***Ubicacion
			jdeFromUnicode(szCamposContenido[iIdxUbic],dsF4211Buf[iLineaEscogida].sdlocn,21,UTF8);//La ubicación del F4211...
			iCamposPosiciones[iIdxUbic] = strlen(szCamposContenido[iIdxUbic]);
			iCamposErrores[iIdxUbic] = 2; //Nos queremos posicionar acá...
			...
			memset(LszLineLocationBuf,'\0',sizeof(LszLineLocationBuf));

			do{ //Mientras haya error...

				iErrorCode1 = 0;
				iErrorCode = 0;
				ZeroMathNumeric(&mnCantIntroducidaBuf);
				ZeroMathNumeric(&mnCantEmbarcarTemp);


				do{ // Loop para obtener una Ubicación valida...

					iErrorCode1 = 0;

					OWDCmp02 (&iPrimeraVez, szPantallaTitulo, (iCamposOffset + 5), iCamposCantidad, iStatusOffset,iInicioEtiquetas, 
									iInicioCampos, iUltimaLinea, iCReqTAB,
									iCamposPosiciones, szCamposEtiquetas, szCamposContenido, iCamposErrores, szPantallaStatusLine,
									iDbgFlg, dlg);
					...
					//***Ubicacion 
					jdeToUnicode(LszLineLocationBuf,szCamposContenido[iIdxUbic],20,UTF8);
					...
					if (fLineQtyBuf == 0) {
						iSalir = 1;
						if (iDbgFlg > 0){
								jdeFprintf(dlg,_J("***Break de Cantidad...\n"));
								jdeFflush(dlg);
							};
						break;//Queremos salir a pedir otra Etiqueta...
					}
					else iSalir = 0;
	...
	//						if (cValidarUbicacion == _J('1')){//Queremos validar la Ubicación...
					...
					if (iDbgFlg > 0) {
							jdeFprintf(dlg,_J("***Validando Ubicación (%ls/%ls)...\n"),
								dsVerifyAndGetItemLocation.szBranchPlant,dsVerifyAndGetItemLocation.szLocation);
							jdeFflush(dlg);
					}
					...
					while (ErrorRec = jdeErrorGetNextDDItemNameInfoEx(lpBhvrCom,lpVoid)){
						iErrorCode = 309;
						iErrorCode1 = 1;
						iCamposErrores[iIdxUbic] = 1; // Seteamos el error...
						jdeFprintf(dlg,_J("***Error(%d): VerifyAndGetItemLocation: %ls...\n"),iErrorCode,ErrorRec->lpszShortDesc);
						jdeFflush(dlg);
					} 
	//						}

				}while (iErrorCode1 != 0); // Loop para obtener una Ubicación válida...

				if (fLineQtyBuf == 0) {
					iSalir = 1;
					if (iDbgFlg > 0){
							jdeFprintf(dlg,_J("***Break de Cantidad 2...\n"));
							jdeFflush(dlg);
						};

					OWDCmp90 (iDbgFlg, dlg);//Limpiamos la 4ta y 5ta pantalla para mostrar la 3ra...
					continue;//Queremos salir a pedir otra Etiqueta...
				}
				else iSalir = 0;
...
//FIN Modificación - 08/02/2010	
				...
				//***Proceso  Lotes...
				iNumLotes = dsF4211Buf[iLineaEscogida].iNumLotes;//Flag de Lotes...
				iErrorCode = 0;
				iErrorCode1 = 0;
				...
				if(LcLotProcess == _J('1')){ //Procesamiento de Lotes...
					...
					MathCopy(&dsCalculateAvailability.mnShortItemNumber,&dsF4211Buf[iLineaEscogida].sditm);
					jdeStrcpy(dsCalculateAvailability.szBranchPlant,dsF4211Buf[iLineaEscogida].sdmcu);
					...
					MathAdd(&mnCantEmbarcarTemp,&dsF4211Buf[iLineaEscogida].mnCantEmbarcar,&mnCantIntroducidaBuf);

					if (MathCompare(&mnCantEmbarcarTemp,&dsF4211Buf[iLineaEscogida].sdsoqs) >= 1){
						iErrorCode = 312;
						iErrorCode1 = 1;
						jdeToUnicode(LszString01,mnCantEmbarcarTemp.String,DIM(LszString01),UTF8);
						jdeToUnicode(LszString02,dsF4211Buf[iLineaEscogida].sdsoqs.String,DIM(LszString02),UTF8);
						jdeFprintf(dlg,_J("***Error(%d): Cantidad Acumulada Incorrecta(%ls>%ls)...\n"),iErrorCode,
									LszString01,LszString02);
						jdeFflush(dlg);
						fLineQtyBuf = MathNumericToDouble(&mnCantEmbarcarTemp);
						fLineQty1Buf = MathNumericToDouble(&dsF4211Buf[iLineaEscogida].sdsoqs);
					...
					if (iErrorCode1 == 0){//No hay errores: procesar EditLine y acumulados...
						...
						if (iNumLotes == 1){//Registro Padre...
							if (iDbgFlg > 0) {
								jdeFprintf(dlg,_J("***Asignando F42UI05--info Lote Padre (múltiples)...\n"));
								jdeFflush(dlg);
							}
							dsF42UI05EditLine.cRecordWritten = _J('0');
							dsF42UI05EditLine.cActionCode = _J('P');
							MathCopy(&dsF42UI05EditLine.mnLineNumber,&dsF4211Buf[iLineaEscogida].sdlnid);
							MathCopy(&dsF42UI05EditLine.mnOriginal_LineNumber_3,&dsF42UI05EditLine.mnLineNumber);
							MathCopy(&dsF42UI05EditLine.mnMultiLocParentOrderQuantity,&dsF4211Buf[iLineaEscogida].sdsoqs);
							MathSubtract(&dsF42UI05EditLine.mnMultiLocQtyShipVariance,
								&dsF42UI05EditLine.mnEnteredShipQuantity,&dsF4211Buf[iLineaEscogida].sdsoqs);
						}
						if (iNumLotes > 1){//Registro Hijo...
							if (iDbgFlg > 0) {
								jdeFprintf(dlg,_J("***Asignando F42UI05--info Lote Hijo (múltiples)...\n"));
								jdeFflush(dlg);
							}
							dsF42UI05EditLine.cRecordWritten = _J('0');  
							dsF42UI05EditLine.cActionCode = _J('S');
							MathAdd(&dsF42UI05EditLine.mnLineNumber,&mnTemp0001Buf,&dsF4211Buf[iLineaEscogida].sdlnid);//Calculamos linea hija...							
							MathCopy(&dsF42UI05EditLine.mnOriginal_LineNumber_3,&dsF4211Buf[iLineaEscogida].mnLineNumberPadre);//No.Linea del padre...
