...
//Modificado :	Luis Capriles,		Fecha:	22/09/2014 - Cambio a procesamiento acumulado por número de líneas con decimales...
...
//Buscamos el item leido en el cache...
	...
	//FIN Modificación - 08/02/2010	
					...
					if(LcLotProcess == _J('1')){ //Procesamiento de Lotes...
						...
						if ( 1 == 2) { //140912 No queremos validar disponibilidad: si está en el pick es porque había...
							if (iDbgFlg > 0) {
						...
						if (iErrorCode1 == 0){//No hay errores: procesar EditLine y acumulados...
							...
							if (iNumLotes == 1){//Registro Padre...
								...
								//ZeroMathNumeric(&dsF42UI05EditLine.mnOriginal_LineNumber_3);
		...
		//Modificado Luis Capriles,22/09/2014 - Cambio a procesamiento acumulado...INICIO
			if (iSalir > 0){//Queremos romper este lazo para pasar a actualizar...
				if (iDbgFlg > 0){
					jdeFprintf(dlg,_J("***Break de Captura de Código de Barra->Procesar...\n"));
						jdeFflush(dlg);
					};
				break; //Queremos procesar este PS!!!!
			}
		...
		if ((iEditLineLines == 1) && (LszUsrEntry[0] == LcYesNoBuf)){
			iSalir = 0;


		}
		else{
			iSalir = 1; //Queremos romper el lazo de EsditLines para pedir otro Pick directamente...
		...
		do {//TRUE
			...
			if (iSalir > 0){//Queremos romper este lazo y no actualizar nada si la respuesta fué N o no hay líneas...
				if (iDbgFlg > 0){
					jdeFprintf(dlg,_J("***Break de Procesar Emnarque: NO se quiere procesar...\n"));
						jdeFflush(dlg);
					};
				break; //Queremos subir a perdir otro PS!!!!
			}

		//Modificado Luis Capriles,22/09/2014 - Cambio a procesamiento acumulado...FIN
			...
			if (iErrorCode1 == 0){
				...
				if (iErrorCode == 0){
					...
					//Modificado Luis Capriles,22/09/2014 - Cambio a procesamiento acumulado...INICIO
					/*140922 Luis Capriles Movido a procesamiento acumulado...
