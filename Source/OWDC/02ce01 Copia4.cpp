...
__declspec(dllexport) int OWDCce01(HENV hEnv,HUSER hUser)
{
	...
	typedef struct {
		...
		MATH_NUMERIC	mnJobNumber;		//Job Number
	} stF4211ce01;
...
#define LineCant 500
	...
	stLineNumHijos		dsF4211LineNumBuf[LineCant];
...
//Buscamos el item leido en el cache...
	...
	//FIN Modificación - 08/02/2010	
					...
					if(LcLotProcess == _J('1')){ //Procesamiento de Lotes...
						...
						/**140917 Si se escanea un lote, puede ser distinto al del pedido...
						if (jdeStrcmp(LszLineLotBuf,dsF4211Buf[j].sdlotn) != 0 ) {// El lote escaneado debe coincidir con el del pedido...
							if (iDbgFlg > 0) {
								iErrorCode = 323;
								iErrorCode1 = 1;
								jdeFprintf(dlg,_J("***Error(%d): Lotes No Coinciden: %ls/%ls)...\n"),iErrorCode,LszLineLotBuf,dsF4211Buf[j].sdlotn);
								jdeFflush(dlg);

								jdeSprintf(LszString01,_J("***Error(%d): Lotes: %ls/%ls)...\n"),iErrorCode,
						...
						/**140912 No queremos validar disponibilidad: si está en el pick es porque había...
						if (iDbgFlg > 0) {
						...
						if (iErrorCode1 == 0){//No hay errores: procesar EditLine y acumulados...
							...
							/**
							if ((iNumLotes == 1) && (1==2)){//Queremos verificar si este Padre no es realmente un Hijo...								
							...
							**/
							...
							if (iNumLotes > 1){//Registro Hijo...								
								...
								ZeroMathNumeric(&dsF42UI05EditLine.mnLineNumber);
			...
			if (iErrorCode1 == 0){
				...
				MathCopy(&dsF42UI05EditLine.mnJobnumberA,&dsF4211Buf[iLineaEscogida].mnJobNumber);
				...
				if (iDbgFlg > 0) {
					...
					jdeFprintf(dlg,_J("***F42UI05EditLine 1-Job(%ls)...\n"),LszString01);
					jdeFflush(dlg);
				}
				...
				if (iErrorCode == 0){
					...
					MathCopy(&dsF4211Buf[iLineaEscogida].mnJobNumber,&dsF42UI05EditLine.mnJobnumberA);
