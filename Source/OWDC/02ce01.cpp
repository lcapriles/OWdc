...
//Modificado:	Luis Capriles,		Fecha:	22/09/2014 - Cambio a procesamiento por número de líneas con decimales...
//
//El programa ce01 es una versión reducida del programa P4205-Confirmación de Embarques.
//1)	Solicita un Número de Pick Slip 
//1.1)		Si no se introduce Pick Slip la ejecución termina.
//1.2)      Se cargan en cache las lineas de pedidos para ese número
//2)	Solicita un Código de Etiqueta 
//2.1)      Se descompone en Item, Lote y UM
//2.2)      Con el Item, se valida si es un código de barras llamando a la función GetBC y trata de localizarlo en el caché.
//2.3)	    Si lo consigue y hay más de una línea, escoge la primera encontrada
//2.3.1)        Una vez localizado el código del producto en el caché, se soliucita cantidad, etc. 
//2.4)      Si no lo consigue, se retorna al punto 2
//2.5)      Si todo está OK, se retorna al punto 2
//3)	Si no se introduce un Código de Etiqueta
//3.1)      Se solicita aprobar el despacho
//3.2)      Se retorna al punto 1
//
#include "ce01.h"
#include <stdio.h>
#include "jde.h"
...
__declspec(dllexport) int OWDCce01(HENV hEnv,HUSER hUser)
{

	HREQUEST			hRequest	= (HREQUEST)NULL;
	HREQUEST			hRequest1	= (HREQUEST)NULL;
...
//		MATH_NUMERIC	mnJobNumber;		//Job Number			LC:22/09/2014 - Cambio a procesamiento por número de líneas con decimales...
	} stF4211ce01;
...
#define LineCant 5000

	stF4211ce01			dsF4211Buf[CacheSize];
	stLineNumHijos		dsF4211LineNumBuf[LineCant] = {0};
	...
	NID					szF4211ColumnsArraySO[1] =	{NID_LNID};
	KEY5_F4211			dsF4211Key5;
	KEY1_F4211			dsF4211Key1;
	SELECTSTRUCT		lpSelectF4211[9];
	SELECTSTRUCT		lpSelectF4211SO[4];
	LPF4108				lpdsF4108 = (LPF4108) NULL;
	LPF4101				lpdsF4101 = (LPF4101) NULL;
	MATH_NUMERIC		mnTempBuf,mnTempBuf1,mnTemp0Buf,mnTemp0001Buf,mnCantIntroducidaBuf,mnCantEmbarcarBuf,
	...
	JCHAR LszLinea[80],LszLin1[64],LszLin2[64],LszUsrEntry[128],
		 ...
		 LszString01[128],LszString02[64],LszString03[64],LszTempBuf[128],* LszDummy0,
	...
	int	 iPickSlipNumberBuf,iRecordsF4211Read,iLineaEscogida,iUPClenBuf,iSCClenBuf, iError1,iError2,
		 iErrorCode,iErrorCode1,iDbgFlg,iItemFoundInCache,iRecordsAvailCache,iEditLineLines,iCantLotesProc,iNumLotes,i,j,k,iDummy, 
	...
	char * szCamposEtiquetas[64], szCamposContenido[64][128], szPantallaTitulo[64], szPantallaStatusLine[64], szDummy[128],
		 * szDummy1,szTempBuf[128], szLin1[5], szLin2[5];
...
*/
	...
	memset((void *)(&dsF4211Key1),(int)(_J('\0')),sizeof(dsF4211Key1));
	...
	if (iDbgFlg > 0) jdeFprintf(dlg,_J("***Abriendo Tabla F4211 para SO...\n"));
	idResult = JDB_OpenTable (hUser,NID_F4211,ID_F4211_ORDER_NUMBER__ORDER_TYPE___A,szF4211ColumnsArraySO,
	                               (ushort)(1),(JCHAR *)NULL,&hRequest1);
	if (idResult == JDEDB_FAILED){
		iErrorCode = 304;
		jdeFprintf(dlg,_J("***Error(%d): JDB_OpenTable(F4211 para SO) failed...\n"),iErrorCode);
		jdeFflush(dlg);
		goto lbFIN;
	}
	...
	*/
	...
	do{ //while (TRUE);
		...
		memset(dsF4211LineNumBuf,'\0',sizeof(dsF4211LineNumBuf));
		...
		//Modificado Luis Capriles, 22/09/2014 - Cambio a procesamiento por número de líneas con decimales... INICIO
		//Hay un PickSlip, determinamos números de líneas...
		//Construye Where del select...
		JDB_ClearSelection(hRequest1);

		//Actualiza la clausula Where ...
		jdeNIDcpy(lpSelectF4211SO[0].Item1.szDict, NID_KCOO);//Compañía...
		jdeNIDcpy(lpSelectF4211SO[0].Item1.szTable, NID_F4211);
		lpSelectF4211SO[0].Item1.idInstance = (ID)0;
		jdeNIDcpy(lpSelectF4211SO[0].Item2.szDict, _J(""));
		jdeNIDcpy(lpSelectF4211SO[0].Item2.szTable, _J(""));
		lpSelectF4211SO[0].Item2.idInstance = (ID)0;
		lpSelectF4211SO[0].lpValue = dsF4211Buf[0].sdkcoo;//Cualquiera sirve... [0] en particular!
		lpSelectF4211SO[0].nValues = (short)1;
		lpSelectF4211SO[0].nAndOr = JDEDB_ANDOR_AND;
		lpSelectF4211SO[0].nCmp = JDEDB_CMP_EQ;

		jdeNIDcpy(lpSelectF4211SO[1].Item1.szDict, NID_DOCO);//Documento...
		jdeNIDcpy(lpSelectF4211SO[1].Item1.szTable, NID_F4211);
		lpSelectF4211SO[1].Item1.idInstance = (ID)0;
		jdeNIDcpy(lpSelectF4211SO[1].Item2.szDict, _J(""));
		jdeNIDcpy(lpSelectF4211SO[1].Item2.szTable, _J(""));
		lpSelectF4211SO[1].Item2.idInstance = (ID)0;
		lpSelectF4211SO[1].lpValue = &dsF4211Buf[0].sddoco;//Cualquiera sirve... [0] en particular!
		lpSelectF4211SO[1].nValues = (short)1;
		lpSelectF4211SO[1].nAndOr = JDEDB_ANDOR_AND;
		lpSelectF4211SO[1].nCmp = JDEDB_CMP_EQ;

		jdeNIDcpy(lpSelectF4211SO[2].Item1.szDict, NID_DCTO);//Tipo Documento...
		jdeNIDcpy(lpSelectF4211SO[2].Item1.szTable, NID_F4211);
		lpSelectF4211SO[2].Item1.idInstance = (ID)0;
		jdeNIDcpy(lpSelectF4211SO[2].Item2.szDict, _J(""));
		jdeNIDcpy(lpSelectF4211SO[2].Item2.szTable, _J(""));
		lpSelectF4211SO[2].Item2.idInstance = (ID)0;
		lpSelectF4211SO[2].lpValue = dsF4211Buf[0].sddcto;//Cualquiera sirve... [0] en particular!
		lpSelectF4211SO[2].nValues = (short)1;
		lpSelectF4211SO[2].nAndOr = JDEDB_ANDOR_AND;
		lpSelectF4211SO[2].nCmp = JDEDB_CMP_EQ;
		
		if (iDbgFlg > 0) jdeFprintf(dlg,_J("***Construyendo Selection Tabla F4211 para SO...\n"));
		idResult = JDB_SetSelection(hRequest1,lpSelectF4211SO,(short)(3),JDEDB_SET_REPLACE);
		if (idResult == JDEDB_FAILED){
			iErrorCode = 305;
			jdeFprintf(dlg,_J("***Error(%d): JDB_SetSelection (F4211 para SO) failed...\n"),iErrorCode);
			jdeFflush(dlg);
			goto lbFIN;
		}
		
		if (iDbgFlg > 0) jdeFprintf(dlg,_J("***Ejecutando Select sobre Tabla F4211 para SO...\n"));
		idResult = JDB_SelectKeyed(hRequest1,(ID) 0,(void *)NULL,(short)0);
		if (idResult == JDEDB_FAILED){
			iErrorCode = 306;
			jdeFprintf(dlg,_J("***Error(%d): JDB_SelectKeyed (F4211 para SO) failed...\n"),iErrorCode);
			jdeFflush(dlg);
			goto lbFIN;
		}
		k = 0;
		do {//Carga en Cache los número de líneas de la SO asociada al Pick Slip escogido...
			if (iDbgFlg > 0) jdeFprintf(dlg,_J("***Ejecutando Fetch sobre Tabla F4211 para SO...\n"));		
			idResult = JDB_Fetch(hRequest1,&dsF4211LineNumBuf[0],(int)0); //Usamos el registro [0] como buffer...

			if (idResult == JDEDB_FAILED)jdeFprintf(dlg,_J("***JDB_Fetch (EOF F4211 para SO) (%d) ...\n"),k);
			else {
				k++;
				ParseNumericString(&mnTempBuf,_J("10"));	
				MathMultiply(&mnTempBuf,&dsF4211LineNumBuf[0].mnLineNumberHijo,&mnTempBuf);
				MathNumericToInt(&mnTempBuf,&iDummy); //Truncamos el número de línea...(1.1xx->10, 1.1xx->11, ...)
				if (iDummy >= LineCant){
					iErrorCode = 307;
					jdeFprintf(dlg,_J("***Error(%d): LineCant(k) > CacheSize!!!\n"),iErrorCode);
					jdeFflush(dlg);
					goto lbFIN;
				}
				if (MathCompare(&dsF4211LineNumBuf[0].mnLineNumberHijo,&dsF4211LineNumBuf[iDummy].mnLineNumberHijo) >= 1){//La que acabamos de leer es mayor...
					if (iDbgFlg > 0){
						//FormatMathNumeric(LszString01,&dsF4211LineNumBuf[0].sddoco);
						FormatMathNumeric(LszString02,&dsF4211LineNumBuf[0].mnLineNumberHijo);
						FormatMathNumeric(LszString03,&dsF4211LineNumBuf[iDummy].mnLineNumberHijo);
						jdeFprintf(dlg,_J("***Cache[%d] Order LNID, LineNumberHijo (%ls,%ls)...\n"),iDummy,LszString02,LszString03);
						
						jdeFflush(dlg);
					}
					MathCopy(&dsF4211LineNumBuf[iDummy].mnLineNumberHijo,&dsF4211LineNumBuf[0].mnLineNumberHijo);
				}
			}
		} while (idResult != JDEDB_FAILED);
		//Modificado Luis Capriles, 22/09/2014 - Cambio a procesamiento por número de líneas con decimales... FIN
...
//				do { //Loop para validar codigo barra... }
				...
				iIdxProd = 0; iIdxDescr = 1; iIdxCant = 2; iIdxLot = 3; iIdxFVcto = 9;
				...
				if (iErrorCode1 == 1){
					...
					if (iErrorCode != 0){ //Hubo error... vamos a quitar ceros para ver...
						...
						if (iDbgFlg > 0) {
							...
							jdeFprintf(dlg,_J("***LszUOMdefaultBuf/LszUOMstdConv 2(%ls/%ls)...\n"),LszUOMdefaultBuf,LszUOMstdConv);
			...
			*/
			...
			do {//while (iItemFoundInCache == 0); Loop para Solicitar producto a procesar... 
				...
				if (iDbgFlg > 0){
					jdeFprintf(dlg,_J("***Inicializando 4ta pantalla datos entrada por Línea...\n"));
					jdeFflush(dlg);
				}
...
lbEditLineERR: //Por ahora tenemos una etiqueta..
					...
					if (iDbgFlg > 0) {
						...
						jdeFprintf(dlg,_J("***LszUOMdefaultBuf/LszUOMstdConv (%ls/%ls)...\n"),LszUOMdefaultBuf,LszUOMstdConv);
						jdeToUnicode(LszString01,mnItemShortIDBuf.String,DIM(LszString01),UTF8);
...
//Buscamos el item leido en el cache...
				...
				if (iError1 == 0){//Lo encontramos.. vamos a validar la cantidad...
					...
					//jdeStrcpy(LszLineUMBuf,LszUOMdefaultBuf);
					jdeStrcpy(LszLineUMBuf,dsF4211Buf[j].sduom); //La UM es la del pedido porque no se puede cambiar: cambiamos la cantidad!


					// Convertimos la UM/Cant de la transacción en la UM/Cant del Pedido porque no se aceptan UM distintas...
					memset ((void *)(&dsGetItemUoMConversionFactor), (int)(_J('\0')), sizeof(DSD4000520));
					jdeStrcpy (dsGetItemUoMConversionFactor.szFromUnitOfMeasure,LszUOMdefaultBuf); //UM de la transacción(según OWDCgbc)...
					jdeStrcpy (dsGetItemUoMConversionFactor.szToUnitOfMeasure,dsF4211Buf[j].sduom); // UM del pedido... 
					...
					while (ErrorRec = jdeErrorGetNextDDItemNameInfoEx(lpBhvrCom,lpVoid)){	  						
						...
						jdeFprintf(dlg,_J("***Error(%d): GetItemUoMConversionFactor: %ls(%ls->%ls)...\n"),iErrorCode,ErrorRec->lpszShortDesc,
									LszUOMdefaultBuf,dsF4211Buf[j].sduom);
						jdeFflush(dlg);
					...
					MathCopy (&mnCantIntroducidaBuf,&dsGetItemUoMConversionFactor.mnQuantityConvertedFromtoTo);//Llevamos la cantidad de la transacción a la UM de la línea...

					if (iDbgFlg > 0) jdeFprintf(dlg,_J("***GetItemUoMConversionFactor - UOMtr->UOM (%ls/%ls)(%s/%s)(...\n"),
												LszUOMdefaultBuf,dsF4211Buf[j].sduom,LszString01,LszString02);


	//Modificado, Luis Capriles, 08/02/2010 - Cambio orden datos detalle.
	//FIN Modificación - 08/02/2010	
					...
					if(LcLotProcess == _J('1')){ //Procesamiento de Lotes...
						...
						if (jdeStrcmp(LszLineLotBuf,_J("")) == 0 ) {// Si no se escanea lote, el lote es el del pedido...
							jdeStrcpy(LszLineLotBuf,dsF4211Buf[j].sdlotn);
						}

						//140917 Si se escanea un lote, puede ser distinto al del pedido...  Si lo es, verificar la disponibilidad!!!
						if (jdeStrcmp(LszLineLotBuf,dsF4211Buf[j].sdlotn) != 0 ) {// El lote escaneado no coincide con el del pedido...
							if (iDbgFlg > 0) {
								jdeFprintf(dlg,_J("***Lotes No Coinciden: %ls/%ls)...\n"),LszLineLotBuf,dsF4211Buf[j].sdlotn);
								jdeFflush(dlg);
							}
							
							if (iDbgFlg > 0) {
								jdeFprintf(dlg,_J("***Procesando Disponibilidad...\n"));
								jdeFflush(dlg);
							}
							...
							//DoubleToMathNumeric(fLineQtyBuf,&mnCantIntroducidaBuf); Ya la ajustamos mas arriba a la UOM...
						
							if (iDbgFlg > 0) {
								jdeToUnicode(LszString01,mnCantIntroducidaBuf.String,DIM(LszString01),UTF8);
								jdeFprintf(dlg,_J("***fLineQtyBuf Cantidad Lote (%lf/%ls)...\n"),fLineQtyBuf,LszString01);
								jdeFflush(dlg);
							}
						...
						if (iErrorCode1 == 0){//No hay errores: procesar EditLine y acumulados...
							...
							/**
							if ((iNumLotes == 1) && (1==2)){//Queremos verificar si este Padre no es realmente un Hijo de una línea sin decimales...								
								iDummy = strlen(dsF4211Buf[j].sdlnid.String);
								if (strcmp(dsF4211Buf[j].sdlnid.String+(iDummy - 3),"000")){//Esto solo se verifica si el número de línea tiene decimales...									
									strcpy(szLin1,dsF4211Buf[j].sdlnid.String);
									szLin1[iDummy-3] = '\0';//truncamos el NroLínea...

									k = 0;									
									for (i = 0;(i < iRecordsF4211Read) && (k == 0); i++){
										strcpy(szLin2,dsF4211Buf[i].sdlnid.String);
										iDummy = strlen(dsF4211Buf[i].sdlnid.String);
										szLin2[iDummy-3] = '\0';//truncamos el NroLínea...
										iDummy = 0;
										if ((jdeStrcmp(LszLineItemBuf,dsF4211Buf[i].sdaitm) == 0) && 
											(dsF4211Buf[i].iStatusRegistro == 2) && //Solo registros procesados...
											 strcmp(szLin1,szLin2) == 0) iDummy = 1; //Y que sean de la misma "línea"...

										if ((iDummy == 1)){
											if (iDbgFlg > 0){
												jdeToUnicode(LszString01,dsF4211Buf[i].sdlnid.String,DIM(LszString01),UTF8);
												jdeToUnicode(LszString02,dsF4211Buf[i].mnLineNumberPadre.String,DIM(LszString01),UTF8);
												jdeFprintf(dlg,_J("***Padre encontrado dsF4211Buf[%d].sdaitm (%ls/ l:%ls p:%ls)...\n"),i,dsF4211Buf[i].sdaitm,
																LszString01,LszString02); 
												jdeFflush(dlg);
											}
											k++;
											iNumLotes = dsF4211Buf[i].iNumLotes;//Flag de Lotes...
											MathCopy(&dsF4211Buf[j].mnLineNumberPadre,&dsF4211Buf[i].mnLineNumberPadre);//No.Linea del padre...
										}
									}				
								}
							}
							**/
							...
							if (iNumLotes == 1){//Registro Padre...  
								...
								//MathCopy(&dsF4211Buf[j].mnLineNumberHijo,&dsF4211Buf[j].sdlnid); //LC:22/09/2014 - Cambio a procesamiento por número de líneas con decimales...

								/**  LC: 22/09/2014 - Cambio a procesamiento por número de líneas con decimales...
								if (LcDecimalCharBuf == _J(',')) ParseNumericString(&mnTempBuf,_J("0,300"));
								else ParseNumericString(&mnTempBuf,_J("0.300"));					
								MathAdd(&dsF4211Buf[j].mnLineNumberHijo,&mnTempBuf,&dsF4211Buf[j].sdlnid);
								**/
							...
							if (iNumLotes > 1){//Registro Hijo...								
								...
								MathAdd(&dsF42UI05EditLine.mnLineNumber,&mnTemp0001Buf,&dsF4211Buf[j].mnLineNumberHijo);//Calculamos linea hija...	MAL!!!

								//Modificado Luis Capriles, 22/09/2014 - Cambio a procesamiento por número de líneas con decimales... INICIO
								ParseNumericString(&mnTempBuf,_J("10"));	
								MathMultiply(&mnTempBuf,&dsF4211Buf[j].sdlnid,&mnTempBuf);
								MathNumericToInt(&mnTempBuf,&iDummy); //Truncamos el número de línea...(1.1xx->10, 1.1xx->11, ...)
								MathAdd(&mnTempBuf1,&mnTemp0001Buf,&dsF4211LineNumBuf[iDummy].mnLineNumberHijo);//Calculamos linea hija...
								MathCopy(&dsF4211LineNumBuf[iDummy].mnLineNumberHijo,&mnTempBuf1);//Actualizamos linea hija...
								ParseNumericString(&mnTempBuf,_J("10"));	
								MathMultiply(&mnTempBuf,&mnTempBuf1,&mnTempBuf);
								MathNumericToInt(&mnTempBuf,&iDummy);
								MathCopy(&dsF42UI05EditLine.mnLineNumber,&mnTempBuf1);//Actualizamos linea hija...
								MathCopy(&dsF4211LineNumBuf[iDummy].mnLineNumberHijo,&mnTempBuf1);//Actualizamos linea hija...
								//Modificado Luis Capriles, 22/09/2014 - Cambio a procesamiento por número de líneas con decimales... FIN
...
//				MathCopy(&dsF42UI05EditLine.mnJobnumberA,&dsF4211Buf[iLineaEscogida].mnJobNumber);  LC:22/09/2014 - Cambio a procesamiento por número de líneas con decimales...
//				ZeroMathNumeric(&dsF42UI05EditLine.mnXT4111LineNumber);
				...
				if (iDbgFlg > 0) {
					jdeFprintf(dlg,_J("***F42UI05EditLine 1...\n"));
					jdeFflush(dlg);
				}
				...
				if (iErrorCode == 0){
					...
					if (iDbgFlg > 0) {
						FormatMathNumeric(LszString01,&dsF42UI05EditLine.mnJobnumberA);
						jdeFprintf(dlg,_J("***F42UI05EditLine OK-Job(%ls)...\n"),LszString01);
						jdeFflush(dlg);
					}
...
//					MathCopy(&dsF4211Buf[iLineaEscogida].mnJobNumber,&dsF42UI05EditLine.mnJobnumberA); LC:22/09/2014 - Cambio a procesamiento por número de líneas con decimales...
...
//							memset((void *) &dsF42UI05EditLine,(int) _J('\0'),sizeof(DSD4200790A));
						//MathCopy(&dsF4211Buf[iLineaEscogida].mnLineNumberHijo,&dsF42UI05EditLine.mnLineNumber); //LC:22/09/2014 - Cambio a procesamiento por número de líneas con decimales...
						...
						if (iDbgFlg > 0) {
							jdeFprintf(dlg,_J("***F42UI05EditLine 2...\n"));
							jdeFflush(dlg);
						}
						...
						if (iDbgFlg > 0) {
							jdeFprintf(dlg,_J("***F42UI05EditLine 3...\n"));
							jdeFflush(dlg);
						}
						idResult = jdeCallObject( _J("F42UI05EditLine"),(LPFNBHVR)NULL,lpBhvrCom1,lpVoid1,&dsF42UI05EditLine,
													(CALLMAP *)NULL,(int)0,(JCHAR *)NULL,(JCHAR *)NULL,(int)0);
					...
					//Modificado Luis Capriles, 22/09/2014 - Cambio a procesamiento por número de líneas con decimales... INICIO
					/**
					if (dsF4211Buf[iLineaEscogida].iStatusRegistro == 2){//Acabamos de completar un alínea... vamos a cerrarla
						if (iDbgFlg > 0){
							jdeFprintf(dlg,_J("***Inicializando 6ta pantalla datos Aceptacion Línea Despacho...\n"));
							jdeFflush(dlg);
						}
						...
						szCamposEtiquetas[0] = "Procesar Linea %d de Embarque?(S/N)...",iLineaEscogida;  
						...
						if (LszUsrEntry[0] == LcYesNoBuf) {
							...
							if (iDbgFlg > 0) {
								jdeFprintf(dlg,_J("***ShipConfirmEndDoc de línea %d...\n",iLineaEscogida));
								jdeFflush(dlg);
							}
							...
							while (ErrorRec = jdeErrorGetNextDDItemNameInfoEx(lpBhvrCom1,lpVoid1)){
								iErrorCode = 315;			
								jdeFprintf(dlg,_J("***Error(%d): ShipConfirmEndDoc línea:%d- %ls...\n"),iErrorCode,iLineaEscogida,ErrorRec->lpszShortDesc);
								jdeFflush(dlg);
							}
							if ((idResult == ER_ERROR) && (iErrorCode == 0)){
								iErrorCode = 315;			
								jdeFprintf(dlg,_J("***Error(%d): ShipConfirmEndDoc línea %d...\n"),iErrorCode,iLineaEscogida);
								jdeFflush(dlg);
							}
							jdeFflush(dlg);
							...
							if (iDbgFlg > 0){
								...
								jdeFprintf(dlg,_J("***Numero Embarque para línea %d(%ls)...\n"),iLineaEscogida,LszString01);
								jdeFflush(dlg);
							}
						} //if (LszUsrEntry[0] == LcYesNoBuf)
					} //if (dsF4211Buf[iLineaEscogida].iStatusRegistro == 2) //Acabamos de completar un alínea... vamos a cerrarla
